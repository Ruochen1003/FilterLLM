...
class MF(BaseColdStartTrainer):
    ...
    def load_emb(self, root, model_name):
        user_file_name = model_name + '_user_emb.pt'
        item_file_name = model_name + '_item_emb.pt'
        user_emb_path = os.path.join(root, user_file_name)
        item_emb_path = os.path.join(root, item_file_name)
        #self.model.load_emb(root, model_name)
        self.user_emb = torch.load(user_emb_path)
        self.item_emb = torch.load(item_emb_path)
...
class Matrix_Factorization(nn.Module):
    ...
    def load_emb(self, root, model_name):
        ...
        user_emb_path = os.path.join(root, user_file_name)
        item_emb_path = os.path.join(root, item_file_name)
        self.embedding_dict['user_emb'] = torch.load(user_emb_path)
        self.embedding_dict['item_emb'] = torch.load(item_emb_path)
